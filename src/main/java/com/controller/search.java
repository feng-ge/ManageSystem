...
public class search {
    ...
    @RequestMapping("/{eid}")
    public void searchresult1(@PathVariable int eid,  HttpServletResponse response) throws IOException {
        File html = new File("F:\\杂\\我学尼玛的习\\期刊稿件管理系统\\fosu\\src\\main\\webapp\\index.html");
        Document document = Jsoup.parse(html, "UTF-8");
        EssayExample EssayExample = new EssayExample();
        EssayExample.or().andIfpublishEqualTo(true).andEclassIdEqualTo(eid);
        List<Essay> list = essayMapper.selectByExample(EssayExample);
        Classify Classify = null;
        User User = null;
